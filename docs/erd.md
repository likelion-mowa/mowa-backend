...
- `demo_session_id`는 로그인마다 발급되는 논리적 데이터 격리 식별자이며 별도 `sessions` 테이블을 사용하지 않습니다.
- `walk_candidates`, `experience_drafts`, `walk_experiences`는 같은 `user_id`라도 `demo_session_id`가 다르면 서로 다른 데모 데이터 영역으로 취급합니다.
- Candidate → Draft → Experience는 동일한 `user_id`와 동일한 `demo_session_id`를 사용해야 하며 소유권 및 세션 정합성은 백엔드 서비스 레이어에서 검증합니다.
...
| `demo_session_id` | 로그인마다 발급되는 논리적 데모 데이터 격리 식별자입니다. 별도 Session Entity나 FK 관계를 만들지 않습니다. |
...
| 별도 테이블 정책 | Calendar, Album, Notification, Push Token, Profile, Refresh Token, Session, Cloudinary Asset 관리 등의 별도 테이블은 MVP에서 생성하지 않습니다. Cloudinary도 ERD Entity로 추가하지 않습니다. |

---

# 4. 사용자 소유권 및 세션 정합성

세 핵심 테이블은 각각 `user_id`와 `demo_session_id`를 보유합니다.
...
`demo_session_id`는 별도 Session 테이블 FK가 아니며, 실제 DB FK는 기존 `user_id`, `candidate_id`, `draft_id` 구조를 유지합니다.

다만 단순 FK만으로 위 값들의 동일성을 직접 보장하는 것은 아니므로 백엔드 서비스 레이어에서 다음 규칙을 검증합니다.
...
`user_id`와 `demo_session_id`는 클라이언트 Request에서 받지 않고 서버가 인증 정보에서 설정합니다.
...
로그인 사용자
      =
walk_candidates.user_id + walk_candidates.demo_session_id
      =
experience_drafts.user_id + experience_drafts.demo_session_id
```

다른 사용자 또는 다른 `demo_session_id`의 Candidate를 이용하여 Draft를 생성할 수 없습니다.
...
experience_drafts.user_id + experience_drafts.demo_session_id
      =
walk_experiences.user_id + walk_experiences.demo_session_id
```

다른 사용자 또는 다른 `demo_session_id`의 Draft를 이용하여 최종 Experience를 생성할 수 없습니다.
...
각 테이블은 `user_id FK + NOT NULL`로 `users.id`를 참조하고, `demo_session_id NOT NULL`로 로그인 세션별 데모 데이터 영역을 구분합니다.
...
같은 `user_id`라도 `demo_session_id`가 다르면 서로 다른 데모 데이터 영역으로 취급합니다.
